...
const FacultyDashboard = () => {
  ...
  const handledownload = () => {
    console.log("Download triggered for:", profile?.name);

    if (!profile?.qr_code) {
      alert("QR Code data not found. Please refresh or contact admin.");
      return;
    }
    ...
    try {
      ...
      const empId = profile.employee_id || "FACULTY";
      const name = profile.name ? profile.name.replace(/\s+/g, "_") : "QR";
      link.download = `Official_QR_${empId}_${name}.png`;
      ...
      alert(
        "Browser blocked the download. Try right-clicking the QR code instead.",
      );
    }
  };
  ...
  const handleSave = async () => {
    setUpdating(true);
    try {
      // Create a copy of profile with the fixed photo URL
      const processedProfile = {
        ...profile,
        profile_photo_url: convertDriveLink(profile.profile_photo_url),
      };

      await facultyService.updateProfile(processedProfile);

      // Update local state with the converted link so the photo updates immediately
  ...
  const convertDriveLink = (url) => {
    ...
    // Regular expression to extract the ID from a Google Drive URL
    const driveRegex = /[-\w]{25,}/;
    const match = url.match(driveRegex);

    if (match && match[0]) {
      const fileId = match[0];
      // This is the most reliable direct-link format for 2026
    ...
    <div className="min-h-screen bg-[#f4f7f9] font-sans text-slate-800">
      ...
      <div className="bg-white border-b-4 border-red-700 px-6 py-4 shadow-sm flex flex-col md:flex-row justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div className="bg-red-700 text-white p-2 font-bold text-xl rounded">
            VI
          </div>
      ...
      <div className="max-w-7xl mx-auto p-6 lg:p-10">
        ...
        <div className="flex justify-between items-end mb-8 border-b-2 border-slate-200 pb-4">
          <div>
            <h2 className="text-2xl font-serif font-bold text-slate-800 uppercase tracking-tight">
              Faculty Information Management System
            </h2>
            <p className="text-slate-500 text-sm">
              Update your academic records and digital presence.
            </p>
          </div>
          <div className="flex gap-2">
            {!isEditing ? (
              <button
                ...
                className="flex items-center gap-2 bg-blue-900 text-white px-5 py-2 text-sm font-bold rounded shadow-md hover:bg-blue-800 transition-all"
              >
              ...
              <>
                <button
                  ...
                  className="px-5 py-2 text-sm font-bold border border-slate-300 rounded hover:bg-slate-50"
                >
                  CANCEL
                </button>
                <button
                  onClick={handleSave}
                  disabled={updating}
                  className="flex items-center gap-2 bg-green-700 text-white px-5 py-2 text-sm font-bold rounded shadow-md hover:bg-green-800 disabled:opacity-50"
                >
                  {updating ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-slate-200 shadow-sm p-6 sticky top-28">
              <div className="w-full aspect-square bg-slate-100 border-2 border-slate-200 mb-4 overflow-hidden relative group">
                {profile.profile_photo_url ? (
                  <img
                    ...
                    alt="Faculty"
                    onError={(e) => {
                      // Fallback if the Drive link is restricted or invalid
                      e.target.src =
                ...
                {isEditing && (
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center p-4 text-center">
                    <p className="text-[10px] text-white font-bold uppercase leading-tight">
                      Paste Google Drive Link Below <br /> (Set to "Anyone with
                      link")
                    </p>
                  </div>
                )}
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-xs font-bold text-red-700 uppercase mt-1">
                  {profile.designation}
                </p>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 text-center">
                ...
                <button
                  onClick={handledownload} // Corrected: removed the arrow function wrapper
                  className="mt-2 w-full flex items-center justify-center gap-2 py-2 px-4 bg-slate-50 border border-slate-200 text-[10px] font-black text-blue-900 hover:bg-blue-900 hover:text-white transition-all rounded"
                >
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <Section
              title="Professional & Academic Profiles"
              icon={GraduationCap}
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <TraditionalInput
                ...
                <TraditionalInput
                  ...
                  icon={Microscope}
                  onChange={(v) =>
                    setProfile({ ...profile, google_scholar: v })
                  }
                />
                <TraditionalInput
                ...
                />
                <TraditionalInput
                  label="Profile Image (Google Drive or Image URL)"
                  value={profile.profile_photo_url}
                  isEditing={isEditing}
                  icon={Camera}
                  placeholder="https://drive.google.com/file/d/..."
                  onChange={(v) => {
                    // Convert it immediately so the preview updates!
                    const converted = convertDriveLink(v);
                    setProfile({ ...profile, profile_photo_url: converted });
                  }}
                />
                {isEditing && (
                  <p className="text-[9px] text-slate-400 mt-2 italic font-medium">
                    Note: If using Google Drive, ensure the file sharing is set
            ...
            <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-slate-50 px-6 py-3 border-b border-slate-200 flex justify-between items-center">
                <h4 className="text-xs font-bold text-blue-900 uppercase flex items-center gap-2">
                  <BookOpen size={14} className="text-red-700" /> Research &
                  Publications
                </h4>
                {isEditing && (
                  <button
              ...
              <div className="p-6">
                {profile.research.length === 0 ? (
                  <p className="text-sm text-slate-400 italic">
                    No publications listed in the official record.
                  </p>
                ) : (
                  <div className="space-y-0 border-l border-r border-t border-slate-200">
                    {profile.research.map((paper, index) => (
                      <div
                        key={index}
                        className="border-b border-slate-200 p-4 hover:bg-slate-50 transition-colors flex flex-col md:flex-row gap-4 items-center"
                      >
                          ...
                          <>
                            ...
                            <button
                              onClick={() =>
                                setProfile({
                                  ...profile,
                                  research: profile.research.filter(
                                    (_, i) => i !== index,
                                  ),
                                })
                              }
                          ...
                          <div className="flex-1">
                            ...
                            {paper.link && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="text-blue-700 text-[10px] font-bold hover:underline flex items-center gap-1 mt-1"
                              >
                                LINK TO PUBLICATION <ExternalLink size={10} />
                              </a>
                            )}
                          </div>
                        )}
...
const TraditionalInput = ({
  ...
  <div className="space-y-1">
    ...
    <div className="relative border-b-2 border-slate-100 focus-within:border-blue-900 transition-colors">
      <Icon
        className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-300"
        size={14}
      />
      <input
